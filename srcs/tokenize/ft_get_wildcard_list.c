...
void	ft_update_list(t_list *curr, t_list *prev, t_list **head, t_list **w_list)
{
	t_list	*next;
	t_list	*last_node;
...
t_list	**ft_get_wildcard_list(char *s)
{
	...
	*wild_list = NULL;
	...
	while (entry)
	{
		...
		{
			sub = ft_substr(entry->d_name, ft_strlen(entry->d_name) - ft_strlen(sufix), ft_strlen(sufix));
			if (ft_strncmp(entry->d_name, prefix, ft_strlen(prefix)) == 0 && ft_strncmp_(sub, sufix, ft_strlen(sufix)) == 0)
			{
