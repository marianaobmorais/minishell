...
pid_t	ft_child_process(int *curr_fds, t_shell *sh, void *curr_node, void *next_node)
{
	pid_t	pid;
	...
	if (pid == 0)
	{
		...
		close(curr_fds[1]);
		ft_exec(((t_exec *)curr_node)->args, sh);
	}
	return (pid);
}
...
void	ft_launcher(void *curr_node, void *next_node, int *curr_fds, t_shell *sh)
{
	...
	{
		sh->prev = curr_node;
		ft_launcher(((t_redir *)curr_node)->next, next_node, curr_fds, sh);
	}
	else if (((t_exec *)curr_node)->type == EXEC)
	{
		...
		pid = ft_child_process(curr_fds, sh, curr_node, next_node);
